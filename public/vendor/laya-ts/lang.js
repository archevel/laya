...
const NON_EN_DIACRITICS = new Set(("àâäãáåçéèêëíìîïñóòôöõøúùûüýÿßæœ" + // Western European
    "ăâîșțşţ" + // Romanian
    "ąćęłńśźż" + // Polish
...
function iterText(state, depth = 0) {
    if (depth > 6 || state == null)
        return [];
...
export function latinProfile(text) {
...
function leafNonEnglish(leaf) {
    ...
    if (det.isEnglish)
        return null;
    if (det.language !== null && det.language !== "en")
        return det;
    const nAlpha = alphaCount(sample);
    if (det.script !== "latin" && det.script !== "unknown") {
        if (nonLatinWords(sample).length > 0 && nAlpha >= NON_LATIN_MIN_LETTERS)
            return det;
        return null;
    }
...
export function isEnglish(state) {
