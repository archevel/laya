...
const SIGNOFF_HEAD = /^\s*(?:best|kind|warmest|warm|many thanks|thanks|thank you|regards|cheers|sincerely)(?:\s+(?:and|&)\s+regards|\s+(?:regards|wishes|again|in advance|a lot|so much|very much))?/i;
...
function isEnglishSignoff(line) {
...
const SIGNATURE_MARKERS = [
    ...
    (l) => /^\s*(atenciosamente|att|abraços?|abs|um abraço|cordialmente|grat[oa]|(muito )?obrigad[oa]s?( desde já| pela atenção)?|(com os melhores )?cumprimentos|saudações|(un )?saludos?( cordiales)?|atentamente|(muchas )?gracias( de antemano)?)[\s,!.]*$/i.test(l),
];
const DEVICE = "iphone|ipad|android|ios|celular|telemóvel|móvil|galaxy|smartphone|samsung|tablet|" +
    "outlook|yahoo|mail|e-?mail|gmail|windows";
const DEVICE_FOOTER = new RegExp("^\\s*((enviad[oa] (do|pelo|pela|via|desde|a partir do)( meu| minha| mi)?|sent from( my)?)" +
    ` (${DEVICE})( (${DEVICE}|para|for|no|na|\\d+))*|(obter o|get) outlook (para|for) (ios|android))[\\s.!]*$`, "i");
...
"(\\b(e-?mail|message|information|communication|transmission|contents?)\\b[^.]{0,60}" +
    ...
    "\\b(meio|medio) ambiente\\b[^.]{0,30}antes de imprimir)", "i");
const SENTENCE = /(?<=[.!?])\s+/;
...
function splitFusedLines(sentence) {
...
function stripDisclaimer(paragraph) {
    if (!DISCLAIMER.test(paragraph))
        return paragraph;
    const parts = paragraph.split(SENTENCE).map((p) => p.trim()).filter(Boolean);
    const pieces = [];
    for (const p of parts) {
        if (DISCLAIMER.test(p))
            pieces.push(...splitFusedLines(p));
        else
            pieces.push(p);
    }
...
export function cleanEmailBody(body, maxChars = 3000) {
    ...
    for (let i = 0; i < src.length; i++) {
        const line = src[i];
        if (QUOTE_HEADERS.some((p) => p.test(line)) && lines.length)
            break;
        if (lines.length && HEADER_FROM_NAME.test(line) && i + 1 < src.length && HEADER_NEXT.test(src[i + 1]))
            break;
        if (ATTRIBUTION_TAIL.test(line) && lines.length) {
            if (ATTRIBUTION_HEAD.test(lines[lines.length - 1]))
                lines.pop();
            break;
        }
    ...
    for (let i = start; i < lines.length; i++) {
        ...
        if ((n <= 40 && SIGNATURE_MARKERS.some((p) => p(lines[i]))) ||
