...
export const CHECKPOINT_IDS = { cls: 50281, sep: 50282, mask: 50284, pad: 50283, unk: 50280 };
...
function byteUnicodeMaps() {
    ...
    const ranges = [[0x21, 0x7e], [0xa1, 0xac], [0xae, 0xff]];
    let k = 0;
...
function bpeWord(chars, rank) {
    if (chars.length >= HEAP_MIN_LEN)
        return bpeWordHeap(chars, rank);
...
function bpeWordHeap(chars, rank) {
    ...
    const offer = (slot) => {
        ...
        while (c > 0) {
            const p = (c - 1) >> 1;
            if (heapRank[p] < r || (heapRank[p] === r && heapSlot[p] < slot))
                break;
        ...
        if (heapLen > 0) {
            ...
            for (;;) {
                ...
                if (r < heapLen && (heapRank[r] < heapRank[l]
                    || (heapRank[r] === heapRank[l] && heapSlot[r] < heapSlot[l])))
                    m = r;
                if (heapRank[m] > lastRank || (heapRank[m] === lastRank && heapSlot[m] > lastSlot))
                    break;
...
export function bpeEncode(vocab, merges, text) {
    ...
    for (const piece of parts) {
        ...
        for (const b of enc.encode(piece))
            chars.push(b2u.get(b) ?? "");
        for (const tok of bpeWord(chars, merges))
            out.push(vocab.get(tok) ?? unkId);
    }
    return out;
}
...
export function metaspaceEncode(vocab, merges, text, unkId, replaces = [[" ", METASPACE_REPLACEMENT]]) {
...
export function parseTokenizerJson(raw) {
    try {
        ...
        if (kind === "metaspace" && replaces.length === 0)
            replaces.push([" ", METASPACE_REPLACEMENT]);
        return {
            vocab, merges,
