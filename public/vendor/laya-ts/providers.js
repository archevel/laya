...
export function resolveRevision(_repoOrId, revision) {
...
function normaliseDigestPath(rel) {
...
async function expectDigest(rel, data, expected) {
    ...
    if (got.toLowerCase() !== String(want).trim().toLowerCase()) {
        throw new Error(`laya-ts: SHA-256 mismatch for ${rel}: expected ${want}, got ${got}. Refusing to load the artifact.`);
    }
}
function toNested(data, dims) {
    ...
    if (dims.length === 0)
        return flat[0];
    const steps = new Array(dims.length);
    for (let d = 0; d < dims.length; d++) {
        let s = 1;
        for (let k = d + 1; k < dims.length; k++)
            s *= dims[k];
        steps[d] = s;
    }
    const rec = (d, off) => {
        if (d === dims.length - 1)
            return flat.slice(off, off + dims[d]);
        const out = new Array(dims[d]);
        for (let i = 0; i < dims[d]; i++)
            out[i] = rec(d + 1, off + i * steps[d]);
        return out;
    };
    return rec(0, 0);
}
...
export function feedHead(ort, hidden, b) {
    ...
    let k = 1;
    for (const r of b.markerPos)
        if (r.length > k)
            k = r.length;
    // Direct flatten into typed arrays; no flat(Infinity)+map intermediates.
    const nH = hidden.length ?? n;
    const S = hidden[0]?.length ?? 1;
    const Hd = hidden[0]?.[0]?.length ?? 1;
...
function applyNumThreads(ort, numThreads) {
    try {
        const raw = numThreads ??
            (typeof process !== "undefined" ? Number(process.env?.["LAYA_THREADS"]) : NaN);
...
async function fetchArrayBuffer(url, onHeaders) {
    const g = globalThis;
    let cache = null;
    let hit = null;
    try {
...
export async function loadNodeBundle(modelDirOrRepo, opts) {
    ...
    try {
        ...
        const cache = path.join(os.homedir(), ".cache", "laya-ts", "hf", modelDirOrRepo.replace(/\//g, "__"), sub ?? "root", ...(revision && revision !== "main" ? [revision] : []));
...
function baseUrlFor(repoOrUrl, subfolder, revision) {
...
export async function loadWebBundle(repoOrUrl, opts) {
    ...
    const fetchVerifiedJson = async (rel) => {
        const buf = await fetchArrayBuffer(`${base}/${rel}`, (response) => {
            const commit = response.headers?.get?.("x-repo-commit");
            if (commit)
                reportedRevision = commit;
        });
        if (opts?.expectedSha256)
            await expectDigest(rel, buf, opts.expectedSha256);
    ...
    try {
        ...
        if (e instanceof Error && e.message.startsWith("laya-ts: SHA-256 mismatch"))
            throw e;
...
export async function createNodeProvider(modelDir, opts) {
    ...
    for (const f of ["encoder.onnx", "head.onnx"]) {
        ...
        if (opts?.expectedSha256)
            await expectDigest(f, await fs.readFile(p), opts.expectedSha256);
    }
    ...
    return {
        runEncoder: async (b) => {
            try {
                ...
                if (isOomError(e))
                    throw new Error(`${e.message} (WebGPU out of memory; WASM fallback already active)`);
                throw e;
            }
        },
        runHead: async (h, b) => {
            try {
                ...
                if (isOomError(e))
                    throw new Error(`${e.message} (out of memory; try fewer questions per call)`);
                throw e;
            }
        },
    };
